...
import com.lxs.bigdata.pay.model.wxpay.WxPayApi;
import com.lxs.bigdata.pay.model.wxpay.request.WxPayRefundRequest;
import com.lxs.bigdata.pay.model.wxpay.request.WxPayUnifiedorderRequest;
import com.lxs.bigdata.pay.model.wxpay.response.WxPayAsyncResponse;
import com.lxs.bigdata.pay.model.wxpay.response.WxPayRefundResponse;
import com.lxs.bigdata.pay.model.wxpay.response.WxPaySyncResponse;
import com.lxs.bigdata.pay.service.BestPayService;
import com.lxs.bigdata.pay.utils.MapUtil;
import com.lxs.bigdata.pay.utils.MoneyUtil;
import com.lxs.bigdata.pay.utils.RandomUtil;
import com.lxs.bigdata.pay.utils.XmlUtil;
...
import static com.lxs.bigdata.pay.constants.WxPayConstants.*;
 ...
 */
@Slf4j
public class WxPayServiceImpl extends AbstractWxPayComponent implements BestPayService {
    ...
    protected String getPayTypeCode() {
        return BestPayTypeEnum.WXPAY_H5.getCode();
    }
    ...
    public PayResponse pay(PayRequest request) {
        ...
        wxRequest.setTradeType(this.switchH5TradeType(request.getPayTypeEnum()));
        ...
        try {
            ...
            log.error("【微信统一支付】失败，原因：{}", e.getMessage(), e);
            throw new WxPayException(BestPayResultEnum.WXPAY_TRADE_ERROR);
        }
        if (!retrofitResponse.isSuccessful()) {
            throw new RuntimeException("【微信统一支付】发起支付, 网络异常");
        }
        WxPaySyncResponse response = retrofitResponse.body();
        if (response == null || !checkResponseCode(response)) {
            throw new WxPayException(BestPayResultEnum.WXPAY_TRADE_ERROR);
        }
    ...
    public PayResponse asyncNotify(String notifyData) {
        ...
        if (!asyncResponse.getReturnCode().equals(WxPayConstants.SUCCESS)) {
            throw new WxPayException("【微信支付异步通知】发起支付, returnCode != SUCCESS, returnMsg = " + asyncResponse.getReturnMsg());
        }
        //该订单已支付直接返回
        ...
        if (!asyncResponse.getResultCode().equals(WxPayConstants.SUCCESS)) {
            throw new WxPayException("【微信支付异步通知】发起支付, resultCode != SUCCESS, err_code = " + asyncResponse.getErrCode() + " err_code_des=" + asyncResponse.getErrCodeDes());
        }
    ...
    public RefundResponse refund(RefundRequest request) {
        ...
        OkHttpClient okHttpClient = new OkHttpClient()
                .newBuilder()
                .sslSocketFactory(wxPayH5Config.getSslContext().getSocketFactory(), this.x509TrustManager())
                .addInterceptor((new HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.BODY)))
                //是否开启缓存
        ...
        Response<WxPayRefundResponse> retrofitResponse;
        try {
            retrofitResponse = call.execute();
        } catch (IOException e) {
            log.error("【微信退款】失败，原因：{}", e.getMessage(), e);
            throw new WxPayException(BestPayResultEnum.WXPAY_REFUND_ERROR);
        }
        if (!retrofitResponse.isSuccessful()) {
            throw new WxPayException("【微信退款】发起退款, 网络异常");
        }
        ...
        if (!response.getReturnCode().equals(WxPayConstants.SUCCESS)) {
            throw new WxPayException("【微信退款】发起退款, returnCode != SUCCESS, returnMsg = " + response.getReturnMsg());
        }
        if (!response.getResultCode().equals(WxPayConstants.SUCCESS)) {
            throw new WxPayException("【微信退款】发起退款, resultCode != SUCCESS, err_code = " + response.getErrCode() + " err_code_des=" + response.getErrCodeDes());
        }
    ...
    @Override
    public PayCancelResponse payCancel(PayCancelRequest request) {
        return null;
    }
